...
public interface IEmpleadoRepository extends JpaRepository<Empleado, UUID> {
    ...
    @Query(value = "SELECT emp.* FROM tb_empleado as emp INNER JOIN tb_cargo cargo ON emp.id_cargo = cargo.\"id\" WHERE emp.estado = 8 AND cargo.nombre_cargo = 'MOTORISTA' AND emp.codigo_empleado NOT IN ( SELECT tsv.codigo_motorista FROM tb_solicitud_vehiculo as tsv WHERE (tsv.fecha_salida BETWEEN :fechaSalida AND :fechaEntrada OR tsv.fecha_entrada  BETWEEN :fechaSalida AND :fechaEntrada) AND tsv.estado  > 2 AND tsv.estado  < 7 ) OR emp.codigo_empleado IN ( SELECT tsv.codigo_motorista FROM tb_solicitud_vehiculo as tsv WHERE (tsv.fecha_salida BETWEEN :fechaSalida AND :fechaEntrada OR tsv.fecha_entrada  BETWEEN :fechaSalida AND :fechaEntrada) AND tsv.codigo_solicitud_vehiculo in ( SELECT ensal.codigo_solicitud_vehiculo FROM tb_entrada_salida as ensal WHERE ensal.estado = 2) )", nativeQuery = true)
